...
public class MovingEnemy {
    ...
    public MovingEnemy(IQ iq, boolean brickPass, boolean wallPass) {
        this.iq = iq;
    ...
    private Direction pathFinding(char[][] matrix, int e_x, int e_y, int p_x, int p_y, IQ iq) {
        ...
        if (iq == IQ.MEDIUM || iq == IQ.HIGH) {
            tracingRange = 25;
        }

        boolean pathExits = false;

        if (Math.abs((e_x - p_x) * (e_y - p_y)) < tracingRange && iq != IQ.LOW) {
